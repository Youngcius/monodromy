...
def canonicalize_power_tuple(power_tuple):
    """Entries in `Polynomial` are stored as (exponent tuple): coefficient,
    where the exponent tuple is required not to have any trailing zeroes.

    This takes a tuple and rewrites it into that form.
    """
...
def get_from_power_tuple(power_tuple, index):
    """Extracts an exponent from an exponent tuple that may have been
    canonicalized to be shorter than the expected length, by removing extra
    zeroes."""
...
class Polynomial:
    ...
    def evaluate(self, variable, value):
        """Replaces the variable indexed at `variable` by the Polynomial
        `value`."""
    ...
    def indefinite_integral(self, variable):
        ...
        The result has vanishing constant term.
        """
        ...
        for k, v in self.coefficient_table.items():
            ...
            integrated_polynomial.coefficient_table[shifted_key] = v / (
                shifted_key[variable]
            )
    ...
    def definite_integral(self, var, lower, upper):
        ...
        return integrated_polynomial.evaluate(
            var, upper
        ) - integrated_polynomial.evaluate(var, lower)

    def derivative(self, var):
        """Produces the derivative against the variable indexed at
        `variable`."""
...
class TrigPolynomial:
    ...
    def evaluate(self, variable, value):
        """Replaces the variable indexed at `variable` by the (linear)
        Polynomial `value`."""
        return TrigPolynomial(
            trig_fn=self.trig_fn,
    ...
    def integrate(self, variable, lower, upper):  # -> List[TrigPolynomial]
        ...
        return [
            ...
            *(
                TrigPolynomial(
                    coefficients=trig_integral.coefficients.derivative(variable) * -1,
    ...
    def to_number(self):
        """Extracts from a constant trig polynomial its literal constant
        value."""
        if self.trig_fn == "sin":
            return self.coefficients.to_number() * math.sin(self.arguments.to_number())
        elif self.trig_fn == "cos":
            return self.coefficients.to_number() * math.cos(self.arguments.to_number())
        else:
