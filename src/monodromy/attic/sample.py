...
def random_alcove_coordinate(denominator=100):
    ...
    third_numerator = randint(
        max(
            -(first_numerator + second_numerator) // 2 + 1,
            -(denominator - 2 * first_numerator) // 2,
        ),
        min(second_numerator + 1, denominator - 2 * first_numerator - second_numerator),
    )
...
def sample_irreducible_circuit(coverage_set, operations, target_gate_polytope):
    """Produces a randomly generated circuit of the prescribed type which
    cannot be rewritten into a circuit of lower cost."""

    operation_gates = {
        operation.operations[0]: qiskit.extensions.UnitaryGate(
            canonical_matrix(
                *alcove_to_positive_canonical_coordinate(
    ...
    while True:
        ...
        point = unitary_to_alcove_coordinate(qiskit.quantum_info.Operator(qc).data)
        computed_depth = cheapest_container(coverage_set, exactly(*point[:-1])).cost
