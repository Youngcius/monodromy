...
Routines for calculating the Haar volume of polytopes expressed in
...
def _haar_volume_tetrahedron(tetrahedron, integrand=None):
    """Integrates the PU(4) Haar form over a 3D tetrahedron expressed in
    positive canonical coordinates, normalized so that CX lies at (pi/2, 0, 0).
    (NOTE: This differs from almost everywhere else in this codebase by a
    factor of 2!)

    See Watts, O'Connor, and Vala's _Metric Structure of the Space of
    Two-Qubit Gates, Perfect Entanglers and Quantum Control_, Equation
    (27), which we've rewritten one step further to remove all products.

    Takes an optional `integrand`, a polynomial expressed in (c1, c2,
    c3), to integrate against the Haar measure.
    """
    ...
    haar_form = sum(
        [
            term.integrate(
                0, Polynomial.from_linear_list([0]), Polynomial.from_linear_list([1])
            )
            for term in haar_form
        ],
        [],
    ...
    return (
        abs(determinant) / (2 / 3 * np.pi) * sum(term.to_number() for term in haar_form)
    )


def _haar_volume_convex_polytope(convex_polytope, integrand=None):
    """Integrates the PU(4) Haar form, expressed in positive canonical
    coordinates, over a 3D convex polytope.

    Takes an optional `integrand`, a polynomial expressed in (c1, c2,
    c3), to integrate against the Haar measure.
    """

    vertices = [[np.pi * x for x in vertex] for vertex in convex_polytope.vertices]
    mapped_tetrahedra = list(
        [
            list(vertices[index] for index in tetrahedron)
            for tetrahedron in convex_polytope.triangulation
        ]
    )
    return sum(
        [
            _haar_volume_tetrahedron(tetrahedron, integrand=integrand)
...
def haar_volume(polytope, integrand=None):
    """Integrates the PU(4) Haar form, expressed in positive canonical
    coordinates, over a 3D polytope.
...
def distance_polynomial_integrals(
    coverage_set: List[CircuitPolytopeData], max_degree=0, chatty=False
):
    """Computes the integrals of dist^n dHaar over the "fresh" part of each
    member of `coverage_set` for exponent n in the range [0, max_degree].

    Returns a dictionary mapping operations tuples from the
    `coverage_set` to a list of calculated integration values.
    """
    ...
    for polytope in coverage_set:
        ...
        positive_complementary_polytope = positive_polytopes_so_far.intersect(
            positive_polytope
        ).reduce()
        negative_complementary_polytope = negative_polytopes_so_far.intersect(
            negative_polytope
        ).reduce()
        ...
        for degree in range(1 + max_degree):
            ...
            positive_polynomial_form = (
                positive_polynomial_form * Polynomial.from_linear_list([0, 1, 1, 1])
            )
            negative_polynomial_form = (
                negative_polynomial_form
        ...
        positive_polytopes_so_far = positive_polytopes_so_far.union(
            positive_polytope
        ).reduce()
        negative_polytopes_so_far = negative_polytopes_so_far.union(
            negative_polytope
        ).reduce()
...
def expected_cost(coverage_set, chatty=False):
    """Calculates the expected cost, using the Haar measure, of a
    `coverage_set` expressed in monodromy coordinates."""
...
def cost_statistics(coverage_set, offset, scale_factor, chatty=False):
    """Calculates a variety of summary statistics involving the expected cost,
    as in the Haar measure, of a `coverage_set` expressed in monodromy
    coordinates.
    ...
    for polytope in coverage_set:
        ...
        square_sigma_overshot += d1 * (
            -2 * (polytope.cost - 3 * offset) * scale_factor * 2 / np.pi
        )
