...
class TestMonodromyHaar(unittest.TestCase):
    ...
    def test_expected_interaction_strength(self):
        ...
        left_expected_interaction_strength = haar_volume(
            positive_canonical_alcove_c2.intersect(
                make_convex_polytope([[1, -2, 0, 0]])
            ),
    ...
    def test_cost_statistics(self):
        """Check that Haar-random cost statistics are computed correctly in a
        known example."""
        ...
        for k in statistics.keys():
            self.assertAlmostEqual(statistics[k], expected[k], delta=epsilon)
