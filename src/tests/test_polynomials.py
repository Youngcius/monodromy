...
class TestMonodromyPolynomials(unittest.TestCase):
    ...
    @ddt.data(
        (
            Polynomial.from_coefficient_list([1, 2]),
    ...
    @ddt.data(
        (
            Polynomial.from_coefficient_list([1, 1, -1]),
            0,
    ...
    @ddt.data(
        ...
        (
            Polynomial.from_linear_list([1, 2, 3, 4]),
            0,
    ...
    @ddt.data(
        (Polynomial.from_coefficient_list([1, 2, 3]), 0),
    ...
    def test_ftc(self, polynomial, variable):
        self.assertEqual(
            polynomial, polynomial.indefinite_integral(variable).derivative(variable)
        )
...
class TestMonodromyTrigPolynomials(unittest.TestCase):
    ...
    @ddt.data(
        (
            Polynomial.from_coefficient_list([1, 2, 3]),
            "sin",
            Polynomial.from_coefficient_list([-np.pi, np.pi]),
            0,
            0,
            1,
    ...
    def test_definite_integrals(
        self, coefficients, trig_fn, arguments, variable, lower, upper, result
    ):
        ...
        self.assertTrue(
            abs(result - sum([primitive.to_number() for primitive in primitives]))
            < epsilon
        )
